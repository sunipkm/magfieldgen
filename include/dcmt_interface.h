...
21701 , 23209 , 44497 } ;
#define MT_MAX_RANGE ( ( double ) 4294967295 )
...
extern inline uint32_t urand_mt ( int id ) //id must range from 0 to mt_statecount
{
  return ( uint32_t ) genrand_mt ( mts [ id ] ) ;
}

extern inline int rand_mt ( int id ) //id must range from 0 to mt_statecount
{
  return ( int ) genrand_mt ( mts [ id ] ) ;
}

extern inline double random_mt ( int id ) //id must range from 0 to mt_statecount
{
  return ( ( genrand_mt ( mts [ id ] ) ) / MT_MAX_RANGE ) ;
}

extern inline uint32_t brand_mt ( int id , int range ) //returns random integers in [ 0 , b ), b > 0 is assumed.
{
...
extern inline uint32_t abrand_mt ( int id , int low , int up ) //returns random integers in [ low , up ), up > low >= 0 is assumed.
{
